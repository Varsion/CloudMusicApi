Aliyun::Sms.configure do |config|
	config.access_key_secret = ENV["ALIYUN_SECRET"]     # 阿里云接入密钥，在阿里云控制台申请
	config.access_key_id = ENV["ALIYUN_KEY"]            # 阿里云接入 ID, 在阿里云控制台申请
	config.action = 'SendSms'                           # 默认设置
	config.format = 'JSON'                              # 短信推送返回信息格式，可以填写 'JSON'或者'XML'
	config.region_id = 'cn-hangzhou'                    # 默认设置，如果没有特殊需要，可以不改
	config.sign_name = "爱学啊"                          # 短信签名，在阿里云申请开通短信服务时申请获取
